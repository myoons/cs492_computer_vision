...
def visualize_keypoints(dataset, descriptor_type):
    ...
    for cls, images in dataset['train_img'].items():
        os.makedirs(f'keypoints/{descriptor_type}/{cls}', exist_ok=True)
        ...
        for idx, img in enumerate(images):
            ...
            cv2.imwrite(f'keypoints/{descriptor_type}/{cls}/{idx}.png', img)


def visualize_histograms(dataset, descriptor_type):
    os.makedirs(f'histograms/{descriptor_type}/train', exist_ok=True)
    os.makedirs(f'histograms/{descriptor_type}/test', exist_ok=True)

    for cls, hist in dataset['train_hist'].items():
        avg = np.average(hist, axis=0)
        plt.bar(range(1, len(avg) + 1), avg)
        plt.savefig(f'histograms/{descriptor_type}/train/{cls}.png')
        plt.close()

    for cls, hist in dataset['test_hist'].items():
        avg = np.average(hist, axis=0)
        plt.bar(range(1, len(avg) + 1), avg)
        plt.savefig(f'histograms/{descriptor_type}/test/{cls}.png')
        plt.close()
